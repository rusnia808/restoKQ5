    ...
    <div class="container-fluid py-5">
        <div class="container py-5">
            <div class="row justify-content-center">
                <div class="col-lg-6">
                    <h1 class="section-title position-relative text-center mb-5">Harga Kaki 5 Rasa Bintang 5</h1>
                </div>
            </div>
           
            <div class="row">
            <?php
                $tampil = mysqli_query($conn, "select * from produk where status_produk='1'");
                while($hasil = mysqli_fetch_array($tampil)){
            ?>
                <div class="col-lg-3 col-md-6 mb-4 pb-2">
                    <div class="product-item d-flex flex-column align-items-center text-center bg-light rounded py-5 px-3">
                        <div class="bg-primary mt-n5 py-3" style="width: 80px;" style="background-color:#E14F7D;">
                            <h4 class="font-weight-bold text-black mb-0"><?php echo $hasil['harga_produk']; ?></h4>
                        </div>
                        <div class="position-relative bg-primary rounded-circle mt-n3 mb-4 p-3" style="width: 150px; height: 150px;">
                            <img class="rounded-circle w-100 h-100" src="dash_admin/<?php echo $hasil['foto_produk'];?>" 
                            style="object-fit: cover;">
                        </div>
                        <h5 class="font-weight-bold mb-4"><?php echo $hasil['nama_produk']; ?></h5>
                        <?php 
                            if(!isset($_SESSION['username'])){?>
                                <a href="login.php" id="order" class="btn btn-sm btn-secondary">Add to Cart</a>
                        <?php
                            }else{?>
                                <a href="" id="order" role="button" data-toggle="modal" 
                                data-target="#orderModal<?php echo $hasil['id_produk']; ?>" 
                                class="btn btn-sm btn-secondary">Add to Cart</a>
                            <?php }
                        ?>
                        <!-- order modal -->
                        
                        <div class="modal fade" id="orderModal<?php echo $hasil['id_produk']; ?>" 
                        tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"aria-hidden="true">
                            <div class="modal-dialog" role="document">
                                <div class="modal-content">
                                <div class="modal-header">
                                                <h4 class="modal-title" id="exampleModalLabel">Update produk</h4>
                                                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                                    <span aria-hidden="true">&times;</span>
                                                </button>
                                            </div>
                                    <div class="modal-body">
                                        <form method="POST" action="product_cart_create.php">
                                            <div class="form-group">
                                                <input type="text" name="id_produk" class="form-control" id="id_produk" 
                                                    value="<?php echo $hasil['id_produk']; ?>" hidden>
                                            </div>
                                            <div class="form-group">
                                            ...
                                            <div class="form-group">
                                                <label for="exampleFormControlInput1">Banyak :</label>
                                                <input type="number" class="form-control" id="banyak_produk" 
                                                name="banyak_produk" min="1" required>
                                            </div>
                                    </div>
